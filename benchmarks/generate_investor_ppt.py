...
def add_metric_row(slide, metrics):
    ...
    for i, (label, value) in enumerate(titles):
        ...
        fill = shape.fill
        fill.solid()
        fill.fore_color.rgb = __import__("pptx").dml.color.RGBColor.from_string(LIGHT_BG)
        shape.line.color.rgb = __import__("pptx").dml.color.RGBColor.from_string("CBD5E1")
        tf = shape.text_frame
        tf.clear()
        p1 = tf.paragraphs[0]
        r1 = p1.add_run()
...
def add_two_col_bullets(slide, left_title, left_bullets, right_title, right_bullets):
    lt = slide.shapes.add_textbox(Inches(0.7), Inches(1.6), Inches(5.4), Inches(0.4))
    rt = slide.shapes.add_textbox(Inches(6.3), Inches(1.6), Inches(5.4), Inches(0.4))
    for box, text in [(lt, left_title), (rt, right_title)]:
        p = box.text_frame.paragraphs[0]
        r = p.add_run()
        r.text = text
        r.font.bold = True
        r.font.size = Pt(18)
        set_rgb(r.font, ACCENT_COLOR)
    add_bullets(slide, left_bullets, left=0.8, top=2.0, width=5.0, height=4.8, font_size=16)
    add_bullets(slide, right_bullets, left=6.4, top=2.0, width=5.0, height=4.8, font_size=16)
...
def add_comparison_table(slide, table_like):
    ...
    for c, h in enumerate(headers):
        ...
        cell.text = h
        cell.fill.solid()
        cell.fill.fore_color.rgb = __import__("pptx").dml.color.RGBColor.from_string("DBEAFE")
...
def build_slide(prs, slide_data, run_id):
    ...
    if slide_data.get("slide_number") == 1:
        ...
        add_metric_row(slide, {
            ...
            "unbeaten_scenarios": metric_map.get("unbeaten_scenarios", "").split("/")[0].strip() if "/" in metric_map.get("unbeaten_scenarios", "") else metric_map.get("unbeaten_scenarios"),
            "scenario_count": metric_map.get("unbeaten_scenarios", "").split("/")[1].strip() if "/" in metric_map.get("unbeaten_scenarios", "") else None,
        })
        add_bullets(slide, [
            "Strong recall and soft recall preserved.",
            "Positive context reduction achieved.",
            "Benchmark suite shows robust policy behavior."
        ], top=3.4, height=2.5, font_size=18)
    elif "table_like" in slide_data:
        add_comparison_table(slide, slide_data["table_like"])
    elif slide_data.get("title") == "Business Value":
        bullets = slide_data.get("bullets", [])
        left = bullets[:4]
        right = bullets[4:]
        add_two_col_bullets(slide, "Economic & Product Value", left, "Research & Strategic Value", right)
    else:
...
def main():
    ...
    if not investor:
        raise RuntimeError("Could not load investor deck summary.")

    run_id = investor.get("run_id", "unknown-run")
    prs = Presentation()
    prs.slide_width = Inches(13.333)
    prs.slide_height = Inches(7.5)

    for slide_data in investor.get("slides", []):
        build_slide(prs, slide_data, run_id)

    pptx_path = OUT_DIR / f"portable-memory-investor-deck-{run_id}.pptx"
    prs.save(str(pptx_path))

    print("Investor PowerPoint generated.")
    print(f"Source investor summary: {investor_path}")
